...
public class LSLTestingInput : MonoBehaviour
{
    ...
    liblsl.StreamInfo[] _streamInfos;
    liblsl.StreamInlet _streamInlet;
    ...
    void FixedUpdate()
    {
        if (_streamInlet == null)
        {
            ...
            _streamInfos = LSL.liblsl.resolve_stream("name", _streamName, 1, 0);
            /*_streamInfos = LSL.LSL.resolve_stream("type", _streamType, 1, 0);*/
            
            if (_streamInfos.Length > 0)
            {
                _streamInlet = new liblsl.StreamInlet(_streamInfos[0]);
